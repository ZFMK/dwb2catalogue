...
class TaxaMerger():
	...
	def markDoublesToSubSpecies(self):
		...
		query = """
		DROP TEMPORARY 
		TABLE IF EXISTS doubles_familycache_temp
		;"""
		...
		query = """
		CREATE TEMPORARY 
		TABLE doubles_familycache_temp (KEY(id), KEY(taxon), KEY(familyCache))
		SELECT t1.id, t1.taxon, t1.familyCache
		FROM `{0}` t1
		INNER JOIN 
		`{0}` t2 ON(
		t1.taxon = t2.taxon 
		...
		query = """
		DROP TEMPORARY 
		TABLE IF EXISTS doubles_no_familycache_temp
		;"""
		log_queries.info(query)
		self.cur.execute(query)
		self.con.commit()
		...
		query = """
		CREATE TEMPORARY 
		TABLE doubles_no_familycache_temp (KEY(id), KEY(taxon))
		SELECT t1.id, t1.taxon
		FROM `{0}` t1
		INNER JOIN 
		`{0}` t2 ON(
		t1.taxon = t2.taxon 
		...
		query = """
		 -- set name_doubles with no familyCache to 6 to prevent that they are put elsewhere in the tree
		UPDATE `{0}` t1 
		INNER JOIN doubles_no_familycache_temp nft
		ON(
			t1.taxon = nft.taxon
			AND t1.id != nft.id
		)
